...
export default function App() {
  ...
  const BreederScreen = () => {
    ...
    const farmList = useMemo(()=>{
      const s = new Set(cattle.map(c=>c.farm||"不明"));
    ...
    return (
      <div style={{paddingBottom:90}}>
        ...
        <div style={{padding:"16px 16px"}}>
          ...
          {/* 導入元フィルター */}
          <div style={{marginBottom:10}}>
            <div style={{color:C.textDim,fontSize:10,fontWeight:700,letterSpacing:1,marginBottom:6}}>導入元（家畜市場）で絞り込み</div>
            <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:4,scrollbarWidth:"none"}}>
              {farmList.map(f=>(
                <button key={f} onClick={()=>{setFarmFilter(f);setSelBreeder(null);}} style={{
                  background: farmFilter===f?`linear-gradient(135deg,${bc},#c04020)`:"#fff",
                  color:       farmFilter===f?"#fff":C.textMid,
                  border:`1.5px solid ${farmFilter===f?bc:C.border}`,
                  borderRadius:20, padding:"6px 14px",
                  fontSize:11, fontWeight:700, cursor:"pointer",
                  whiteSpace:"nowrap", flexShrink:0,
                }}>
          ...
          <div style={{color:C.textDim,fontSize:11,marginBottom:12}}>
            {farmFilter!=="all"&&<span style={{background:`${bc}18`,color:bc,borderRadius:20,padding:"3px 10px",fontWeight:700,marginRight:6}}>{farmFilter}</span>}
            {breederData.length}農家 / {breederData.reduce((s,b)=>s+b.head,0)}頭
          </div>
            ...
            return (
              <div key={s.breeder} style={{marginBottom:10}}>
                <div onClick={()=>setSelBreeder(open?null:s.breeder)} style={{
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <div style={{width:28,height:28,borderRadius:"50%",flexShrink:0,background:idx===0?"#f5a623":idx===1?"#aaa":idx===2?"#cd7f32":C.cardSub,display:"flex",alignItems:"center",justifyContent:"center",color:idx<3?"#fff":C.textDim,fontWeight:900,fontSize:12}}>{idx+1}</div>
                  ...
                  {open&&(
                    <div style={{marginTop:14,borderTop:`1px solid ${C.border}`,paddingTop:12}}>
                      {s.cows.map(c=>{
                        const dg_c=calcDG(c.weights);
                        return (
                          <div key={c.id} onClick={e=>{e.stopPropagation();goDetail(c.id);}} style={{background:"#fff",border:`1px solid ${C.border}`,borderRadius:14,padding:"12px 14px",marginBottom:8,cursor:"pointer",boxShadow:C.shadow}}>
                            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6,alignItems:"center"}}>
                              <TagDisplay tag={c.tag} size={11} highlightSize={15} color={C.accent}/>
                              <div style={{display:"flex",gap:4}}>
                                {c.farm&&<Tag label={c.farm} color={bc}/>}
                                <Tag label={c.sex} color={c.sex==="雌"?"#e06090":C.purple}/>
                              </div>
                            </div>
                            {c.pedigree?.sire?.name&&<div style={{fontSize:11,color:C.textDim,marginBottom:6}}>🐂 {c.pedigree.sire.name}{c.pedigree?.dam?.sire?.name&&`　母父: ${c.pedigree.dam.sire.name}`}</div>}
                            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>
                              {[{label:"DG",val:dg_c?`${dg_c.toFixed(2)}`:null,color:C.accent},{label:"BMS",val:c.result?.bms!=null?`${c.result.bms}`:null,color:C.red},{label:"枝肉DG",val:c.result?.dg?`${c.result.dg}`:null,color:C.green}].map(({label,val,color})=>(
                                <div key={label} style={{background:C.cardSub,borderRadius:8,padding:"5px 8px"}}>
                            ...
                            {c.result&&<div style={{marginTop:8,background:C.amberLight,borderRadius:8,padding:"6px 10px",display:"flex",gap:8,alignItems:"center"}}><Tag label={c.result.grade} color={C.amber}/><span style={{color:C.amber,fontSize:12,fontWeight:700}}>{fmtMoney(c.result.sellPrice)}</span></div>}
                          </div>
                        );
                      })}
                    </div>
          ...
          {breederData.length===0&&<div style={{textAlign:"center",padding:32,color:C.textDim,fontSize:13}}>データがありません</div>}
        </div>
      </div>
    );
  };
